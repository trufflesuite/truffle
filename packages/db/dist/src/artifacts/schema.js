...
exports.schema = graphql_tools_1.mergeSchemas({
    schemas: [
        ...
        graphql_tools_1.transformSchema(json_1.schema, [
            new graphql_tools_1.FilterRootFields((_, rootField) => rootField !== "contract"),
            new graphql_tools_1.FilterRootFields((_, rootField) => rootField !== "contractNames")
        ]),
        `type Query {
        ...
        ContractInstance: {
          ...
          networks {
            networkObject {
              ...
              id
                ...
                resolve: obj => {
                    const { networks } = obj;
                    const result = networks.map(({ networkObject: { address } }) => address)[0];
                    return result;
                }
            },
            network: {
          ...
          networks {
            ...
            contract: {
                ...
                resolve: ({ name }, _, context, info) => info.mergeInfo.delegateToSchema({
                    schema: json_1.schema,
                    operation: "query",
                    fieldName: "contract",
                    args: { name },
