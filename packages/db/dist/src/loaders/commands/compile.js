...
function* generateCompileLoad(result, { directory }) {
  ...
  const getCurrent = function* (name, type) {
    return yield* projects_1.generateProjectNameResolve(
      types_1.toIdObject(project),
      name,
      type
    );
  };
  ...
  const compilations = yield* compilations_1.generateCompilationsLoad(
    loadableCompilations
  );
  ...
  for (const [compilationIndex, compilation] of compilations.entries()) {
    ...
    const contracts = yield* contracts_1.generateContractsLoad(
      loadableContracts
    );
    const nameRecords = yield* nameRecords_1.generateNameRecordsLoad(
      contracts,
      "Contract",
      getCurrent
    );
    yield* projects_1.generateProjectNamesAssign(
      types_1.toIdObject(project),
      nameRecords
    );
  }
...
function processResultCompilation({ sourceIndexes, contracts }) {
  const contractsBySourcePath = contracts
    .map(contract => [contract.sourcePath, contract])
    .reduce(
      (obj, [sourcePath, contract]) =>
