...
exports.generateBytecodesLoad = exports.AddBytecodes = void 0;
const types_1 = require("@truffle/db/loaders/types");
const add_graphql_1 = require("./add.graphql");
Object.defineProperty(exports, "AddBytecodes", { enumerable: true, get: function () { return add_graphql_1.AddBytecodes; } });
/**
 * @dev pre-condition: every contract should have both bytecodes
 */
function* generateBytecodesLoad(compilation) {
    ...
    const flattenedContracts = compilation.sources
        .map(({ contracts }) => contracts)
        .flat();
    // now pluck all the create/call bytecodes and concat them (creates first)
    const { createBytecodes, callBytecodes } = flattenedContracts.reduce(({ createBytecodes, callBytecodes }, { deployedBytecode: callBytecode, bytecode: createBytecode }) => ({
    ...
    // submit
    const result = yield {
        request: add_graphql_1.AddBytecodes,
        variables: { bytecodes }
    };
    const addedBytecodes = result.data.workspace.bytecodesAdd.bytecodes.map(types_1.toIdObject);
...
exports.generateBytecodesLoad = generateBytecodesLoad;
