...
class ArtifactsLoader {
    ...
    load() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield Promise.all(compilations.map(({ id }) => __awaiter(this, void 0, void 0, function* () {
                const { data: { workspace: { compilation: { compiler, processedSources } } } } = yield this.db.query(compilations_1.GetCompilation, { id });
                const networks = yield this.loadNetworks(project.id, result.compilations[compiler.name].contracts, this.config["artifacts_directory"], this.config["contracts_directory"]);
                const processedSourceContracts = processedSources
                    .map(processedSource => processedSource.contracts)
                    .flat();
                const contracts = result.compilations[compiler.name].contracts.map(({ contractName }) => processedSourceContracts.find(({ name }) => name === contractName));
    ...
    loadNameRecords(projectId, nameRecords) {
        return __awaiter(this, void 0, void 0, function* () {
            const nameRecordsResult = yield this.db.query(nameRecords_1.AddNameRecords, {
                nameRecords: nameRecords
            });
            let { data: { workspace: { nameRecordsAdd } } } = nameRecordsResult;
            const projectNames = nameRecordsAdd.nameRecords.map(({ id: nameRecordId, name, type }) => ({
            ...
            const projectNamesResult = yield this.db.query(projects_1.AssignProjectNames, {
                projectNames
            });
        });
    }
    ...
    loadNetworks(projectId, contracts, artifacts, workingDirectory) {
        return __awaiter(this, void 0, void 0, function* () {
            const networksByContract = yield Promise.all(contracts.map(({ contractName, bytecode }) => __awaiter(this, void 0, void 0, function* () {
                const name = contractName.toString().concat(".json");
                const artifactsNetworksPath = fse.readFileSync(path_1.default.join(artifacts, name));
                const artifactsNetworks = JSON.parse(artifactsNetworksPath.toString())
                    .networks;
                let configNetworks = [];
                if (Object.keys(artifactsNetworks).length) {
                    const config = config_1.default.detect({ workingDirectory: workingDirectory });
                    for (let network of Object.keys(config.networks)) {
                        ...
                        if (networkId) {
                            ...
                            if (filteredNetwork.length > 0) {
                                const transaction = yield web3.eth.getTransaction(filteredNetwork[0][1]["transactionHash"]);
                                const historicBlock = {
                                    height: transaction.blockNumber,
                                    hash: transaction.blockHash
                                };
                                const networksAdd = yield this.db.query(networks_1.AddNetworks, {
                                    networks: [
                                        {
                                            name: network,
                                            networkId: networkId,
                ...
                const nameRecords = yield Promise.all(configNetworks.map((network, index) => __awaiter(this, void 0, void 0, function* () {
                    //check if there is already a current head for this item. if so save it as previous
                    let current = yield this.resolveProjectName(projectId, "Network", network.name);
                    return {
                        name: network.name,
                        type: "Network",
                        resource: {
    ...
    getNetworkLinks(network, bytecode) {
        ...
        if (network.links) {
            networkLink = Object.entries(network.links).map(link => {
                let linkReferenceIndexByName = bytecode.linkReferences.findIndex(({ name }) => name === link[0]);
                let linkValue = {
                    value: link[1],
                    linkReference: {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const instances = networksArray.map((networks, index) => {
                ...
                const contractInstancesByNetwork = networks.map(network => {
                    let createBytecodeLinkValues = this.getNetworkLinks(network, contracts[index].createBytecode);
                    let callBytecodeLinkValues = this.getNetworkLinks(network, contracts[index].callBytecode);
                    let instance = {
