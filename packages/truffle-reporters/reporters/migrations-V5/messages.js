  ...
  steps(kind, data){
    ...
    const kinds = {
      ...
      deployed: () => {
        ...
        if(!reporter.migration.dryRun) output +=
        ...
        if (reporter.confirmations !== 0) output +=
          self.underline(`Pausing for ${reporter.confirmations} confirmations...`);
      ...
      // Libraries
      linking: () => {
        let output = self.underline(`Linking`) +
      ...
      // Batch
      many: () =>
        self.underline(`Deploying Batch`),
