...
import { abiSizeInfo } from "lib/allocate/abi";
...
function* decodeAbiStructByPosition(
  ...
  info: Evm.Types.EvmInfo,
  options: Decoding.DecoderOptions = {}
): Generator<Decoding.DecoderRequest, Values.StructResult, Uint8Array> {
  const {
    userDefinedTypes,
    allocations: { abi: allocations }
  } = info;
...
function* decodeAbiTupleByPosition(
  ...
  info: Evm.Types.EvmInfo,
  options: Decoding.DecoderOptions = {}
): Generator<Decoding.DecoderRequest, Values.TupleResult, Uint8Array> {
