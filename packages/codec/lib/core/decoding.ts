...
export function* decodeCalldata(
  ...
  if (allocation === undefined) {
    return {
      ...
      abi: context.hasFallback
        ? CodecUtils.AbiUtils.fallbackAbiForPayability(context.payable)
        : null,
      data: CodecUtils.Conversion.toHexString(info.state.calldata),
      decodingMode: "full" as const
    };
  ...
  if (isConstructor) {
    return {
      ...
      bytecode: CodecUtils.Conversion.toHexString(
...
export function* decodeEvent(
  ...
  const contractContext = CodecUtils.ContextUtils.findDecoderContext(
    info.contexts,
    codeAsHex
  );
