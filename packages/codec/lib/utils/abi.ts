...
export namespace AbiUtils {
  ...
  export function getFallbackEntry(
    abi: AbiTypes.Abi
  ...
  export function abiTupleSignature(
    parameters: AbiTypes.AbiParameter[]
  ): string {
  ...
  export function abisMatch(
    ...
    switch (entry1.type) {
      ...
      case "event":
        return (
          abiSignature(entry1) ===
          abiSignature(<AbiTypes.FunctionAbiEntry | AbiTypes.EventAbiEntry>(
            entry2
          ))
        );
      case "constructor":
