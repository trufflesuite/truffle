...
function parameterToAbi(
  ...
  let internalType: string = Ast.Utils.typeStringWithoutLocation(node);
  //is this an array? if so use separate logic
  if (Ast.Utils.typeClass(node) === "array") {
    ...
    let arraySuffix = Ast.Utils.isDynamicArray(node)
      ? `[]`
      : `[${Ast.Utils.staticLength(node)}]`;
    return {
      name,
  ...
  if (Ast.Utils.typeClass(node) === "struct") {
    let id = Ast.Utils.typeId(node);
    let referenceDeclaration = referenceDeclarations[id];
    if (referenceDeclaration === undefined) {
      let typeToDisplay = Ast.Utils.typeString(node);
...
function toAbiType(
  ...
  let basicType = Ast.Utils.typeClassLongForm(node); //get that whole first segment!
  switch (basicType) {
    case "contract":
      return "address";
    case "struct":
    ...
    case "enum":
      let referenceId = Ast.Utils.typeId(node);
...
export function getterInputs(node: Ast.AstNode): Ast.AstNode[] {
  ...
  while (
    Ast.Utils.typeClass(node) === "array" ||
    Ast.Utils.typeClass(node) === "mapping"
  ) {
    let keyNode = Ast.Utils.keyDefinition(node); //note: if node is an array, this spoofs up a uint256 definition
    inputs.push({ ...keyNode, name: "" }); //getter input params have no name
    switch (Ast.Utils.typeClass(node)) {
      case "array":
        node = node.baseType;
        break;
      case "mapping":
...
function getterParameters(
  ...
  let inputs: Ast.AstNode[] = [];
  while (
    Ast.Utils.typeClass(baseNode) === "array" ||
    Ast.Utils.typeClass(baseNode) === "mapping"
  ) {
    let keyNode = Ast.Utils.keyDefinition(baseNode); //note: if baseNode is an array, this spoofs up a uint256 definition
    inputs.push({ ...keyNode, name: "" }); //again, getter input params have no name
    switch (Ast.Utils.typeClass(baseNode)) {
      case "array":
  ...
  if (Ast.Utils.typeClass(baseNode) === "struct") {
    let id = Ast.Utils.typeId(baseNode);
    let referenceDeclaration = referenceDeclarations[id];
    if (referenceDeclaration === undefined) {
      let typeToDisplay = Ast.Utils.typeString(baseNode);
      throw new Common.UnknownUserDefinedTypeError(
        id.toString(),
        typeToDisplay
      );
    ...
    let outputs = referenceDeclaration.members.filter(
      member =>
        Ast.Utils.typeClass(member) !== "array" &&
        Ast.Utils.typeClass(member) !== "mapping"
    );
