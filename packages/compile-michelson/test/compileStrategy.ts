...
describe("michelson compiler", function () {
  ...
  it("compiles michelson contract", async function () {
    ...
    contracts.forEach(contract => {
      assert.equal(contract.architecture, "tezos", "Contract architecture is tezos");
      assert.equal(contract.compiler.name, "@truffle/compile-michelson", "Compiler name is correct");
      assert.doesNotThrow(() => JSON.parse((contract as TezosCompiledContract).michelson), "Generates valid Michelson");
    });

    assert.equal(contracts.length, 2, "Correct number of Michelson contracts compiled");
  });
});
