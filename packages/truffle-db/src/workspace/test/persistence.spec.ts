...
describe("Persisted data", () => {
  ...
  test("Directories are saved for all resources", async () =>
  {
    ...
    const executionResult = await wsClient.execute(GetBytecode, { id: expectedBytecodeId }, false);
    expect(executionResult.bytecode).toBe(null);

    const executionResultPersisted = await wsClient.execute(GetBytecode, { id: expectedBytecodeId }, true);
    expect(executionResultPersisted.bytecode.bytes).toEqual(Migrations.bytecode);
